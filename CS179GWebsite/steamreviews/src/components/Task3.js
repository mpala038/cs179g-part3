  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/task3?page=${page}&sortBy=${sortBy}&appName=${appName}`);
        console.log(`Fetching data from: http://localhost:5000/task3?page=${page}&sortBy=${sortBy}&appName=${appName}`); // Log the URL
...
which can lead to biases in their reviews?</h2>
      ...
      <div className="sort-options">
        ...
        <select id="sort-by" value={sortBy} onChange={handleSortChange}>
          <option value="desc">Descending Average Enjoyment Score</option>
          <option value="asc">Ascending Average Enjoyment Score</option>
        </select>
      </div>
        <table>
          <thead>
